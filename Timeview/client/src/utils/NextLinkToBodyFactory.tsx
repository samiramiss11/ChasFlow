...
const NextLinkToBodyFactory = ({confirm_success}: SectionLayoutProps): Batch => {
  const listToBatch = {
...
const batchToOrder = {
  ...
  buttons: [{text: 'BEKRÄFTA', icon:null}, {text:'TILLBAKA',icon:null}],
...
const BadDeveloper = {
  ...
  linkDestination: JOURNY_LINSK_CONSTANTS.ONBOARDING_STEP1
    ...
    const pageInTransactionGroup = location.pathname;
            const bookingprefixPageIsVisited = `/` + JOURNY_LINSK_CONSTANTS.TRANSACTION_STEP1 
    ...
    switch (pageInTransactionGroup) {
      case bookingprefixPageIsVisited:
        return listToBatch;
      case `${bookingprefixPageIsVisited}/` + JOURNY_LINSK_CONSTANTS.TRANSACTION_STEP2:
        return batchToOrder;
      case `${bookingprefixPageIsVisited}/` + JOURNY_LINSK_CONSTANTS.TRANSACTION_STEP3:
